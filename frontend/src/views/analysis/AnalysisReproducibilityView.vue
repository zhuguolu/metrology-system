<template>
  <div class="repro-shell">
    <section class="analysis-report-hero analysis-report-hero-repro">
      <div>
        <div class="analysis-report-kicker">Reproducibility</div>
        <h1 class="analysis-report-title">再现性结果报告页</h1>
        <p class="analysis-report-desc">突出操作者差异、交互作用和 AV 结论，把核心判断提前到首屏，方便先读结论再看明细。</p>
      </div>
      <div class="analysis-report-badges">
        ...
        <span class="analysis-report-badge primary">{{ result?.summaryLocal || '等待计算' }}</span>
      </div>
    </section>
...
.repro-card { border-radius: 22px; border: 1px solid rgba(226,232,240,.94); box-shadow: 0 16px 34px rgba(15,23,42,.06); }
.repro-chart-panel { border-radius: 18px; box-shadow: inset 0 1px 0 rgba(255,255,255,.72); }
...
.analysis-report-kicker { display:inline-flex; align-items:center; min-height:28px; padding:0 12px; border-radius:999px; background:rgba(79,70,229,.1); color:#4f46e5; font-size:12px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; }
...
.analysis-report-badge.primary { color:#4f46e5; background:rgba(224,231,255,.92); border-color:rgba(165,180,252,.9); }
@media (max-width: 768px) {
