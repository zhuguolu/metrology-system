<template>
  <div class="query-shell">
    <div class="mobile-query-head calibration-mobile-query-head">
      <div class="mobile-query-row">
        <el-input v-model="search" class="mobile-query-search" placeholder="搜索名称、编号、责任人" clearable @input="onFilter" @clear="onFilter" />
      </div>
      <div class="mobile-query-actions calibration-mobile-query-actions">
    ...
    <!-- 筛选栏 -->
    <div class="filter-bar calibration-filter-bar" :class="{ 'mobile-filter-hidden': isMobile && !showMobileFilters }">
      <div v-if="!isMobile" class="filter-group">
        <div class="filter-label">搜索</div>
        <el-input v-model="search" placeholder="搜索名称/编号/责任人..." clearable size="default" style="width:200px" @input="onFilter" @clear="onFilter" />
      </div>
      ...
      <div class="filter-group">
        ...
        <el-select v-model="filterUseStatus" size="default" style="width:120px" disabled>
          <el-option :value="CALIBRATION_USE_STATUS" :label="CALIBRATION_USE_STATUS" />
        </el-select>
      </div>
      <div class="filter-group">
      ...
      <div class="filter-actions calibration-filter-actions">
    ...
    <div class="batch-bar">
      ...
      <div class="batch-actions">
        ...
        <el-button v-if="authStore.canRecordCalibration" size="small" type="primary" :disabled="!selectedIds.length" @click="openBatchCalib">批量记录校准</el-button>
      </div>
    </div>
    ...
    <div class="mobile-list">
      ...
      <div v-for="d in paged" :key="d.id" class="m-card calibration-mobile-card">
        <div class="m-card-row calibration-mobile-card-head">
          <div class="calibration-mobile-card-title-wrap">
            <input type="checkbox" :checked="isSelected(d.id)" @change="toggleSelection(d.id)" />
            <div class="m-card-title">{{ d.name }}</div>
          </div>
          <span :class="['tag', vTag(d.validity), 'calibration-mobile-validity']">{{ d.validity || '-' }}</span>
        </div>
        <div class="m-card-meta calibration-mobile-meta">
          <div class="m-card-meta-item calibration-mobile-meta-item"><span class="calibration-mobile-meta-label">编号</span><b>{{ d.metricNo || '-' }}</b></div>
          <div class="m-card-meta-item calibration-mobile-meta-item"><span class="calibration-mobile-meta-label">部门</span><b>{{ d.dept || '-' }}</b></div>
          <div class="m-card-meta-item calibration-mobile-meta-item"><span class="calibration-mobile-meta-label">责任人</span><b>{{ d.responsiblePerson || '-' }}</b></div>
          <div class="m-card-meta-item calibration-mobile-meta-item"><span class="calibration-mobile-meta-label">上次校准</span><b>{{ d.calDate || '-' }}</b></div>
          <div class="m-card-meta-item calibration-mobile-meta-item calibration-mobile-meta-item-span"><span class="calibration-mobile-meta-label">下次校准</span>
            <b :class="{ 'text-danger': d.validity==='失效', 'text-warning': d.validity==='即将过期' }">{{ d.nextDate || '-' }}</b>
          </div>
        </div>
        <div class="m-card-footer calibration-mobile-card-footer">
          <div class="mobile-card-kpi calibration-mobile-card-kpi">
            <span :class="['tag', usTag(d.useStatus), 'calibration-mobile-status']">{{ d.useStatus || '正常' }}</span>
          </div>
          <div class="m-card-actions calibration-mobile-actions">
            <button v-if="authStore.canRecordCalibration" class="action-btn action-btn-view" @click="openQuickEdit(d)">快改</button>
            <button v-if="authStore.canRecordCalibration" class="action-btn action-btn-edit" @click="openCalib(d)">校准</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="page-pagination">
      <el-pagination
        ...
        @size-change="() => { page = 1; loadDevices() }"
      />
    </div>
      ...
      <div class="modal-box modal-sm quick-edit-modal">
        ...
        <div class="modal-body quick-edit-body">
          <div class="quick-edit-intro">
            手机端可直接更新常用校准字段，复杂情况再进入完整校准记录。
          </div>
          <div class="form-group">
            <label class="form-label">使用部门</label>
            <input v-model="quickEditForm.dept" list="quick-calib-dept-list" placeholder="选择或输入部门" />
            <datalist id="quick-calib-dept-list">
        ...
        <div class="modal-box modal-sm mobile-tools-sheet">
          ...
          <div class="modal-body">
            <div class="mobile-tools-grid">
              ...
              <button class="btn btn-outline mobile-tools-btn" @click="runMobileAction(exportCal)">
                <span class="mobile-tools-btn-title">导出当前</span>
                <span class="mobile-tools-btn-desc">导出当前校准列表</span>
              </button>
              <button class="btn btn-outline mobile-tools-btn mobile-tools-btn-wide" @click="runMobileAction(exportAll)">
                ...
                <span class="mobile-tools-btn-desc">导出全部校准管理数据</span>
              </button>
            </div>
          </div>
        </div>
...
const CALIBRATION_USE_STATUS = '正常'
const search = ref(''), filterDept = ref(''), filterValidity = ref('')
const filterDateFrom = ref(''), filterDateTo = ref(''), filterUseStatus = ref(CALIBRATION_USE_STATUS)
...
const cf = reactive({ id:null, name:'', calDate:'', cycle:12, calibrationResult:'合格', remark:'' })
...
function restoreCalibrationCache() {
  const cached = calibrationCache.restore()
  if (!cached) return
  ...
  filterUseStatus.value = CALIBRATION_USE_STATUS
  ...
  totalItems.value = Number(cached.totalItems) || devices.value.length
...
function onFilter() { page.value = 1; clearSelection(); loadDevices() }
function resetFilter() {
  search.value = ''; filterDept.value = ''; filterValidity.value = ''
  filterDateFrom.value = ''; filterDateTo.value = ''; filterUseStatus.value = CALIBRATION_USE_STATUS
...
async function loadDevices() {
  try {
    const res = await deviceApi.listPaged({
      ...
      useStatus: CALIBRATION_USE_STATUS,
...
function openBatchCalib() {
  ...
  cf.cycle = 12
  cf.remark = ''
  const selected = devices.value.filter(d => selectedIds.value.includes(d.id))
  ...
  try {
    if (batchMode.value) {
      ...
      await Promise.all(selectedIds.value.map(id =>
        deviceApi.updateCalibration(id, { calDate:cf.calDate, cycle:cf.cycle, calibrationResult:(cf.calibrationResult || (devices.value.find(d => d.id === id && d.useStatus !== '正常') ? '不合格' : '合格')), remark:cf.remark })
      ))
      clearSelection(); closeModal()
      showToast(`已批量更新 ${count} 条校准记录`)
    } else {
      await deviceApi.updateCalibration(cf.id, { calDate:cf.calDate, cycle:cf.cycle, calibrationResult:(cf.calibrationResult || defaultResultByUseStatus(devices.value.find(d => d.id === cf.id)?.useStatus)), remark:cf.remark })
      closeModal()
      showToast('校准记录已保存')
    }
    loadDevices()
...
async function exportAll() {
  try {
    const r = await deviceApi.exportCalibration({ useStatus: CALIBRATION_USE_STATUS })
    const url = URL.createObjectURL(r.data)
...
useResumeRefresh(loadDevices)

onMounted(async () => {
  restoreCalibrationCache()
  syncViewport()
...
<style scoped>
.calibration-filter-bar { margin-bottom: 0; }
.batch-bar {
...
.calibration-mobile-query-head { gap: 8px; }
.calibration-mobile-query-actions {
  justify-content: flex-end;
  gap: 6px;
}
.calibration-mobile-query-actions > * { flex: 0 0 auto !important; }
.calibration-mobile-query-actions :deep(.el-button) {
  min-height: 32px;
  padding: 0 12px;
  border-radius: 10px;
  font-size: 12.5px;
...
.calibration-mobile-card-head {
  margin-bottom: 6px;
  align-items: center;
  gap: 8px;
}
.calibration-mobile-card-title-wrap {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.calibration-mobile-validity,
.calibration-mobile-status {
  font-size: 11px;
  padding: 2px 8px;
  border-radius: 999px;
}
.calibration-mobile-meta {
  gap: 5px 10px;
  margin-bottom: 8px;
}
...
.calibration-mobile-meta-item-span { grid-column: 1 / -1; }
.calibration-mobile-meta-label {
  color: var(--text-muted);
  flex-shrink: 0;
}
.calibration-mobile-meta-item b {
  font-size: 12px;
  font-weight: 700;
  color: var(--text);
  min-width: 0;
...
.calibration-mobile-card-kpi { gap: 6px; }
.calibration-mobile-actions {
  width: auto;
  margin-left: auto;
  gap: 6px;
}
...
@media (max-width: 768px) {
  ...
  .calibration-filter-bar.mobile-filter-hidden { display: none !important; }
  .calibration-mobile-meta {
    grid-template-columns: 1fr 1fr;
  }
  ...
  .calibration-filter-bar .filter-group {
    min-width: 0;
    width: auto;
    gap: 4px;
  }
  .calibration-filter-bar .filter-label {
    font-size: 11.5px;
    margin-bottom: 0;
  }
  .calibration-filter-bar :deep(.el-select),
  .calibration-filter-bar :deep(.el-input) {
    width: 100% !important;
  }
  .calibration-filter-bar :deep(.el-input__wrapper) {
    min-height: 34px;
    padding: 0 10px;
  }
  .batch-bar {
